...
void inner_kernel_v6(int M, int N, int K, double alpha, double *A, int LDA, double *B, int LDB,  double *C, int LDC)
{
       ...
       kstep = min(ks, K - kpos);
        ...
        for(int mpos = 0; mpos < M; mpos += ms)
        {
           mstep = min(ms, M - mpos);
            ...
            inner_kernel_v6(mstep, N, kstep, alpha, &A(mpos, kpos), LDA, &B(kpos, 0), LDB, &C(mpos, 0), LDC); 
